...
type manifestFetcher interface {
	Fetch(ctx context.Context, ref dreference.Named) ([]types.ImageInspect, error)
}
...
func GetImageData(a *types.AuthInfo, name string) ([]types.ImageInspect, *registry.RepositoryInfo, error) {
	...
	endpoints, err := registryService.LookupPullEndpoints(dreference.Domain(repoInfo.Name))
	if err != nil {
		return nil, nil, err
	}
...
func getAuthConfig(a *types.AuthInfo, index *registryTypes.IndexInfo) (engineTypes.AuthConfig, error) {
	...
	confFile, err := config.Load(config.Dir())
	if err != nil {
