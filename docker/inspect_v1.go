...
func (mf *v1ManifestFetcher) Fetch(ctx context.Context, ref dreference.Named) ([]types.ImageInspect, error) {
	if _, isCanonical := ref.(dreference.Canonical); isCanonical {
...
func (mf *v1ManifestFetcher) fetchWithSession(ctx context.Context, ref dreference.Named) ([]types.ImageInspect, error) {
	var (
	...
	if err != nil {
		if strings.Contains(err.Error(), "HTTP code: 404") {
			return nil, fmt.Errorf("Error: image %s not found", dreference.Path(mf.repoInfo.Name))
		}
	...
	tagged, isTagged := ref.(dreference.NamedTagged)
	...
	if isTagged {
		...
		tagged, _ := ref.(dreference.NamedTagged)
		tag = tagged.Tag()
		tagsList[tagged.Tag()] = tagID
	}
